  ...
  if (button == BTN_LEFT) {
	...
	if (window->wl.cursorPosY < _GLFW_DECORATION_WIDTH)
	  ...
	  if (window->wl.zxdg.toplevel) {
	    zxdg_toplevel_v6_move(window->wl.zxdg.toplevel, _glfw.wl.seat,
				  serial);
	  }
	}
	break;
	...
	else if (window->wl.cursorPosX >
		 window->wl.width + _GLFW_DECORATION_WIDTH)
	...
	xdg_toplevel_resize(window->wl.xdg.toplevel, _glfw.wl.seat, serial,
      ...
      if (window->wl.zxdg.toplevel) {
	zxdg_toplevel_v6_resize(window->wl.zxdg.toplevel, _glfw.wl.seat, serial,
				edges);
      }
    }
  } else if (button == BTN_RIGHT) {
    if (window->wl.decorations.focus != mainWindow &&
	(window->wl.xdg.toplevel || window->wl.zxdg.toplevel)) {
      if (window->wl.xdg.toplevel) {
	...
	zxdg_toplevel_v6_show_window_menu(
	    window->wl.zxdg.toplevel, _glfw.wl.seat, serial,
...
static const struct wl_seat_listener seatListener = {
    seatHandleCapabilities, seatHandleName,
};

static void dataOfferHandleOffer(void* data, struct wl_data_offer* dataOffer,
				 const char* mimeType) {}
...
static void dataDeviceHandleEnter(void* data, struct wl_data_device* dataDevice,
				  uint32_t serial, struct wl_surface* surface,
...
static void wmBaseHandlePing(void* data, struct xdg_wm_base* wmBase,
			     uint32_t serial) {
...
static void zxdg_shell_ping(void* data, struct zxdg_shell_v6* shell,
			    uint32_t serial) {
  zxdg_shell_v6_pong(shell, serial);
}

static const struct zxdg_shell_v6_listener zxdgShellListener = {
    zxdg_shell_ping,
};
...
static void registryHandleGlobal(void* data, struct wl_registry* registry,
  ...
  if (strcmp(interface, "wl_compositor") == 0) {
    ...
    zxdg_shell_v6_add_listener(_glfw.wl.zxdgShell, &zxdgShellListener, NULL);
...
int _glfwPlatformInit(void) {
  ...
  _glfw.wl.xkb.state_serialize_mods = (PFN_xkb_state_serialize_mods)_glfw_dlsym(
      _glfw.wl.xkb.handle, "xkb_state_serialize_mods");
  ...
  if (!_glfw.wl.wmBase && !_glfw.wl.zxdgShell) {
    _glfwInputError(GLFW_PLATFORM_ERROR,
...
void _glfwPlatformTerminate(void) {
  ...
  if (_glfw.wl.zxdgShell) zxdg_shell_v6_destroy(_glfw.wl.zxdgShell);
